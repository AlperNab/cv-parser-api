# Cv Parser Api — Standalone Real GUI Implementation
...
Default URL: `http://127.0.0.1:9114`

## What is inside this project folder

- `app/` — FastAPI backend for this project.
- `static/` — elegant browser GUI.
- `plugins/cv-parser-api.json` — this project’s own feature/customization/input schema.
...
- `country_format` — country format (select)
- `skill_taxonomy` — skill taxonomy (text)
- `seniority_model` — seniority model (select)
- `redaction_fields` — redaction fields (text)
...
- `resume_files` — Resume files (text) required
