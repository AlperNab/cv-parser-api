...
**Domain:** `HR / Resume Parsing` · **Suite:** `HR & Recruiting Suite` · **Accent:** `#fbbf24`
...
**Cv Parser Api** is a standalone, browser-based AI workflow app for **HR / Resume Parsing**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Resume files → normalized candidate profile.
...
|---|---:|:---:|---|---|
| `resume_files`<br>Resume files | text | Yes | — | Affects input: Resume files. |
...
| `country_format`<br>country format | select | No | United States | Affects customization: country format. |
| `skill_taxonomy`<br>skill taxonomy | text | No | — | Affects customization: skill taxonomy. |
| `seniority_model`<br>seniority model | select | No | intern | Affects customization: seniority model. |
| `redaction_fields`<br>redaction fields | text | No | — | Affects customization: redaction fields. |
...
- **country format**: United States, United Kingdom, Egypt, UAE, Saudi Arabia, Germany, Custom
- **seniority model**: intern, junior, mid-level, senior, lead, executive
...
uvicorn app.main:app --host 127.0.0.1 --port 9114
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9114`. |
...
```bash
curl http://127.0.0.1:9114/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9114/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9114 uvicorn app.main:app --host 0.0.0.0 --port 9114
```
...
- `plugins/cv-parser-api.json` for plugin metadata.
